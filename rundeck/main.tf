locals {
  ...
  tags        = ["rundeck", var.environment, "devops"]
  rundeck_url = var.rundeck_url != "" ? var.rundeck_url : "http://${module.network.vm_elastic_ip_address}:4440"
}
...
resource "arubacloud_project" "this" {
  ...
  description = "Rundeck job automation (${var.environment})"
  tags        = local.tags
}
...
module "network" {
  ...
  vm_ingress_ports = {
    ...
    admin_ui = { port = "4440", cidr = var.admin_cidr }
  }
}
