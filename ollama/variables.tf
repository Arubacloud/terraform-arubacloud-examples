...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'ollama')."
  type        = string
  default     = "ollama"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO8A16 (8 vCPU / 16 GB) is the minimum for 7B models; larger models require more RAM. 7B models need ~4–6 GB free RAM."
  type        = string
  default     = "CSO8A16"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Model files are large — a 7B model is ~4 GB, a 13B is ~8 GB. 100 GB allows several models."
  type        = number
  default     = 100

  validation {
    condition     = var.vm_disk_size_gb >= 50
    error_message = "vm_disk_size_gb must be at least 50 GB for model storage."
  }
}
...
variable "api_cidr" {
  description = "CIDR allowed to reach the Ollama REST API on port 11434. Restrict to your app servers — do not expose to the internet without authentication."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "preload_models" {
  description = "List of Ollama model names to pull at bootstrap time (e.g. ['llama3.2', 'nomic-embed-text']). Each pull may take several minutes."
  type        = list(string)
  default     = []
}
