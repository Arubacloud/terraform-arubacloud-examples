locals {
  ...
  tags        = ["ollama", var.environment, "ai-ml"]
}
...
resource "arubacloud_project" "this" {
  ...
  description = "Ollama local LLM inference server (${var.environment})"
  tags        = local.tags
}
...
module "network" {
  ...
  vm_ingress_ports = {
    ...
    api = { port = "11434", cidr = var.api_cidr }
  }
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      preload_models = var.preload_models
    })
  }
