...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'bind')."
  type        = string
  default     = "bind"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO1A2 (1 vCPU / 2 GB) is more than enough for BIND9."
  type        = string
  default     = "CSO1A2"
}
...
variable "dns_cidr" {
  description = "CIDR allowed to send DNS queries on port 53 (UDP and TCP). Restrict to your VPN tunnel CIDR or client network to prevent open-resolver abuse."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "upstream_dns_1" {
  description = "Primary upstream DNS resolver (forwarder)."
  type        = string
  default     = "1.1.1.1"
}

variable "upstream_dns_2" {
  description = "Secondary upstream DNS resolver (forwarder)."
  type        = string
  default     = "1.0.0.1"
}
