...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'haproxy')."
  type        = string
  default     = "haproxy"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO2A4 (2 vCPU / 4 GB) provides headroom for high connection counts."
  type        = string
  default     = "CSO2A4"
}
...
variable "web_cidr" {
  description = "CIDR allowed to reach the HTTP proxy on port 80. Default is open to the internet."
  type        = string
  default     = "0.0.0.0/0"
}

variable "admin_cidr" {
  description = "CIDR allowed to reach the HAProxy stats page on port 8404. Restrict to your management IP."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "stats_password" {
  ...
  validation {
    ...
    error_message = "stats_password must be at least 8 characters."
  }
}

variable "backends" {
  description = "List of backend server addresses in host:port format (e.g. ['10.0.0.1:80', '10.0.0.2:80']). Leave empty to deploy HAProxy without configured backends — it will return 503 until backends are added."
  type        = list(string)
  default     = []
}
