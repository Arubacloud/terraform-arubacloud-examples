...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'owui')."
  type        = string
  default     = "owui"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO2A4 (2 vCPU / 4 GB) is sufficient for the Open WebUI interface."
  type        = string
  default     = "CSO2A4"
}
...
variable "ollama_base_url" {
  description = "URL of the Ollama server (e.g. 'http://10.0.0.10:11434'). Leave empty to run Open WebUI in standalone mode (uses built-in OpenAI-compatible backends)."
  type        = string
  default     = ""
}

variable "openai_api_key" {
  description = "OpenAI API key for direct OpenAI access. Leave empty if using Ollama or another backend."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "webui_secret_key" {
  description = "Secret key for Open WebUI session signing (min 32 characters). Auto-generated if empty."
  type        = string
  default     = ""
  sensitive   = true
}
