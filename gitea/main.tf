...
resource "arubacloud_project" "this" {
  ...
  description = "Gitea self-hosted Git (${var.environment})"
  tags        = local.tags
}
...
resource "arubacloud_database" "gitea" {
  count = var.enable_mysql ? 1 : 0
...
resource "arubacloud_dbaasuser" "gitea" {
  count = var.enable_mysql ? 1 : 0
...
resource "arubacloud_databasegrant" "gitea" {
  count = var.enable_mysql ? 1 : 0

  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this[0].id
  database   = arubacloud_database.gitea[0].id
  user_id    = arubacloud_dbaasuser.gitea[0].id
  role       = "liteadmin"
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  depends_on = [
    ...
    arubacloud_databasegrant.gitea,
  ]
}
