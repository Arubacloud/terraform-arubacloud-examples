...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'coredns')."
  type        = string
  default     = "coredns"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO1A2 (1 vCPU / 2 GB) is more than sufficient for CoreDNS."
  type        = string
  default     = "CSO1A2"
}
...
variable "dns_cidr" {
  description = "CIDR allowed to send DNS queries on port 53 (UDP and TCP). Should match your VPN tunnel CIDR or client network. Default opens to all — restrict in production."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "upstream_dns_1" {
  description = "Primary upstream DNS resolver for forwarding."
  type        = string
  default     = "1.1.1.1"
}

variable "upstream_dns_2" {
  description = "Secondary upstream DNS resolver for forwarding."
  type        = string
  default     = "1.0.0.1"
}

variable "coredns_version" {
  description = "CoreDNS release version to install (e.g. '1.11.3')."
  type        = string
  default     = "1.11.3"
}
