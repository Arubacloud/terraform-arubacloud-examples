...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'cs')."
  type        = string
  default     = "cs"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO1A2 (1 vCPU / 2 GB) is sufficient for the CrowdSec agent."
  type        = string
  default     = "CSO1A2"
}
...
variable "enroll_key" {
  description = "CrowdSec Console enrollment key (from app.crowdsec.net). Leave empty to skip enrollment."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "collections" {
  description = "List of CrowdSec collections to install (e.g. 'crowdsecurity/linux', 'crowdsecurity/nginx')."
  type        = list(string)
