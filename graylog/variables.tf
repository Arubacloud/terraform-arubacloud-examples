...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'gl')."
  type        = string
  default     = "gl"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO4A8 (4 vCPU / 8 GB) is the minimum for Graylog + OpenSearch + MongoDB co-located."
  type        = string
  default     = "CSO4A8"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Log volume can grow quickly — 100 GB is a reasonable starting point."
  type        = number
  default     = 100

  validation {
    condition     = var.vm_disk_size_gb >= 50
    error_message = "vm_disk_size_gb must be at least 50 GB for Graylog log storage."
  }
}
...
variable "admin_cidr" {
  description = "CIDR allowed to reach the Graylog web UI on port 9000."
  type        = string
  default     = "0.0.0.0/0"
}

variable "syslog_cidr" {
  description = "CIDR allowed to send logs via syslog (port 1514 UDP/TCP) and GELF (port 12201 UDP)."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "graylog_admin_password" {
  ...
  validation {
    ...
    error_message = "graylog_admin_password must be at least 8 characters."
  }
}
...
variable "graylog_secret" {
  ...
  validation {
    ...
    error_message = "graylog_secret must be at least 16 characters."
  }
}
