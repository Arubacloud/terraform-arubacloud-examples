locals {
  ...
  tags               = ["graylog", var.environment, "observability"]
  admin_pass_b64     = base64encode(var.graylog_admin_password)
  graylog_secret_b64 = base64encode(var.graylog_secret)
}
...
resource "arubacloud_project" "this" {
  ...
  description = "Graylog centralized log management (${var.environment})"
  tags        = local.tags
}
...
module "network" {
  ...
  vm_ingress_ports = {
    ...
    webui  = { port = "9000", cidr = var.admin_cidr }
    syslog = { port = "1514", cidr = var.syslog_cidr }
    gelf   = { port = "12201", cidr = var.syslog_cidr }
  }
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      ...
      graylog_secret_b64 = local.graylog_secret_b64
