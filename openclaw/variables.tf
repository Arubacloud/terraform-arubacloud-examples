...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'oclaw')."
  type        = string
  default     = "oclaw"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO2A4 (2 vCPU / 4 GB) is sufficient for OpenClaw."
  type        = string
  default     = "CSO2A4"
}
...
variable "openai_api_key" {
  description = "OpenAI API key for Claude/GPT model access."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "openclaw_password" {
  ...
  validation {
    condition     = var.openclaw_password == "" || length(var.openclaw_password) >= 12
