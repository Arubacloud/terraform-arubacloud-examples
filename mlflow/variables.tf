...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO2A4 (2 vCPU / 4 GB) is sufficient for the tracking server."
  type        = string
  default     = "CSO2A4"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Artifact files (models, plots) are stored here — size according to your needs."
  type        = number
  default     = 40
...
variable "dbaas_flavor" {
  description = "Managed MySQL DBaaS flavor for MLflow experiment tracking storage."
  type        = string
  default     = "DBO2A8"
}
...
variable "mlflow_admin_user" {
  description = "Username for HTTP Basic Auth on the MLflow UI (nginx htpasswd)."
  type        = string
  default     = "admin"
}
...
variable "mlflow_admin_password" {
  ...
  validation {
    ...
    error_message = "mlflow_admin_password must be at least 8 characters and must not contain newlines."
  }
}

variable "mlflow_version" {
  description = "MLflow version to install (e.g. '2.16.0'). See https://pypi.org/project/mlflow/."
  type        = string
  default     = "2.16.0"
}

variable "domain" {
  description = "Custom domain for HTTPS (e.g. 'mlflow.example.com'). Leave empty to use the Elastic IP over HTTP."
  type        = string
  default     = ""
}
