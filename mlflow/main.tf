...
resource "arubacloud_project" "this" {
  ...
  description = "MLflow experiment tracking and model registry (${var.environment})"
  tags        = local.tags
}
...
# ── DBaaS (MySQL 8.0 — MLflow tracking store) ─────────────────────────────────
...
resource "arubacloud_database" "mlflow" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  name       = "mlflow"
}

resource "arubacloud_dbaasuser" "mlflow" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  username   = "mlflow"
...
resource "arubacloud_databasegrant" "mlflow" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  database   = arubacloud_database.mlflow.id
  user_id    = arubacloud_dbaasuser.mlflow.id
  role       = "liteadmin"
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      ...
      db_name                   = arubacloud_database.mlflow.name
      db_user                   = arubacloud_dbaasuser.mlflow.username
      db_password_b64           = local.db_password_b64
      mlflow_admin_user         = var.mlflow_admin_user
      mlflow_admin_password_b64 = local.mlflow_admin_password_b64
  ...
  depends_on = [
    ...
    arubacloud_databasegrant.mlflow,
  ]
}
