...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'ha')."
  type        = string
  default     = "ha"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO2A4 (2 vCPU / 4 GB) handles a typical Home Assistant install with a few dozen integrations."
  type        = string
  default     = "CSO2A4"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Increase if you plan to store large media or recorder history."
  type        = number
  default     = 32
...
variable "admin_cidr" {
  description = "CIDR allowed to reach the Home Assistant web UI on port 8123. Restrict to your management IP or VPN tunnel CIDR."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "timezone" {
  description = "Timezone for Home Assistant (e.g. 'Europe/Rome', 'UTC'). See https://en.wikipedia.org/wiki/List_of_tz_database_time_zones"
  type        = string
  default     = "UTC"
}
