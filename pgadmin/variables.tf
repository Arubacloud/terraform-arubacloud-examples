...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'pgadmin')."
  type        = string
  default     = "pgadmin"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO1A2 (1 vCPU / 2 GB) is sufficient for pgAdmin."
  type        = string
  default     = "CSO1A2"
}
...
variable "admin_cidr" {
  description = "CIDR allowed to reach the pgAdmin web UI on port 80. Never expose pgAdmin to the public internet — restrict to your management IP."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "pgadmin_email" {
  ...
  validation {
    ...
    error_message = "pgadmin_email must be a valid email address."
  }
}
...
variable "pgadmin_password" {
  ...
  validation {
    ...
    error_message = "pgadmin_password must be at least 8 characters."
  }
}
