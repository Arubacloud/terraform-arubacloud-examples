locals {
  ...
  tags        = ["drupal", var.environment, "cms-web"]
...
resource "arubacloud_project" "this" {
  ...
  description = "Drupal CMS (${var.environment})"
  tags        = local.tags
}
...
# Restrict MySQL to the VM's IP only
...
resource "arubacloud_database" "drupal" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  name       = "drupal"
}

resource "arubacloud_dbaasuser" "drupal" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  username   = "drupal"
...
resource "arubacloud_databasegrant" "drupal" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  database   = arubacloud_database.drupal.id
  user_id    = arubacloud_dbaasuser.drupal.id
  role       = "liteadmin"
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      ...
      db_name           = arubacloud_database.drupal.name
      db_user           = arubacloud_dbaasuser.drupal.username
  ...
  depends_on = [
    ...
    arubacloud_databasegrant.drupal,
  ]
}
