...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'os')."
  type        = string
  default     = "os"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO8A16 (8 vCPU / 16 GB) recommended — OpenSearch requires 16 GB RAM for production workloads."
  type        = string
  default     = "CSO8A16"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. OpenSearch indexes can grow quickly — 100 GB is a reasonable starting point."
  type        = number
  default     = 100

  validation {
    condition     = var.vm_disk_size_gb >= 50
    error_message = "vm_disk_size_gb must be at least 50 GB for OpenSearch."
  }
}
...
variable "admin_cidr" {
  description = "CIDR allowed to reach the OpenSearch REST API on port 9200. Never expose OpenSearch directly to the internet."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "opensearch_version" {
  description = "OpenSearch Docker image tag (e.g. '2', '2.15', '2.15.0')."
  type        = string
  default     = "2"
}
