...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'gitlab')."
  type        = string
  default     = "gitlab"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO8A16 (8 vCPU / 16 GB) is the minimum for GitLab CE."
  type        = string
  default     = "CSO8A16"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. GitLab repos, artifacts, and LFS grow over time — 100 GB is a reasonable start."
  type        = number
  default     = 100

  validation {
    condition     = var.vm_disk_size_gb >= 50
    error_message = "vm_disk_size_gb must be at least 50 GB for GitLab CE."
  }
}
...
variable "ssh_cidr" {
  description = "CIDR allowed to reach the admin SSH port 22. Restrict to your IP in production."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "gitlab_root_password" {
  ...
  validation {
    ...
    error_message = "gitlab_root_password must be at least 8 characters."
  }
}

variable "letsencrypt_email" {
  description = "Email address for Let's Encrypt certificate notifications. Leave empty to disable auto-TLS."
  type        = string
  default     = ""
}
