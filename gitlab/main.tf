locals {
  ...
  tags          = ["gitlab", var.environment, "devops"]
  root_pass_b64 = base64encode(var.gitlab_root_password)
  external_url  = var.letsencrypt_email != "" ? "https://${var.gitlab_hostname}" : "http://${var.gitlab_hostname}"
}
...
resource "arubacloud_project" "this" {
  ...
  description = "GitLab CE DevOps platform (${var.environment})"
  tags        = local.tags
}
...
module "network" {
  ...
  vm_ingress_ports = {
    ...
    gitssh = { port = "2222", cidr = "0.0.0.0/0" }
  }
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      root_pass_b64     = local.root_pass_b64
