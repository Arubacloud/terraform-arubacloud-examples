...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'adguard')."
  type        = string
  default     = "adguard"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO1A2 (1 vCPU / 2 GB) is sufficient for AdGuard Home."
  type        = string
  default     = "CSO1A2"
}
...
variable "dns_cidr" {
  description = "CIDR allowed to send DNS queries on port 53 (UDP and TCP). Should match your WireGuard VPN tunnel CIDR or client network. Default opens to all — restrict in production."
  type        = string
  default     = "0.0.0.0/0"
}

variable "admin_cidr" {
  description = "CIDR allowed to reach the AdGuard Home web UI on port 80. Should match your VPN tunnel CIDR or management IP."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "adguard_password" {
  description = "Password for the AdGuard Home admin web interface (min 8 characters)."
  type        = string
  sensitive   = true
  default     = "ChangeMe123!"

  validation {
    condition     = length(var.adguard_password) >= 8
    error_message = "adguard_password must be at least 8 characters."
  }
}
...
variable "adguardhome_version" {
  description = "AdGuard Home release version to install (e.g. '0.107.52')."
  type        = string
  default     = "0.107.52"
}
