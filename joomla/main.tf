...
resource "arubacloud_project" "this" {
  ...
  description = "Joomla CMS (${var.environment})"
  tags        = local.tags
}
...
resource "arubacloud_database" "joomla" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  name       = "joomla"
}

resource "arubacloud_dbaasuser" "joomla" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  username   = "joomla"
...
resource "arubacloud_databasegrant" "joomla" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  database   = arubacloud_database.joomla.id
  user_id    = arubacloud_dbaasuser.joomla.id
  role       = "liteadmin"
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      ...
      db_name        = arubacloud_database.joomla.name
      db_user        = arubacloud_dbaasuser.joomla.username
  ...
  depends_on = [
    ...
    arubacloud_databasegrant.joomla,
  ]
}
