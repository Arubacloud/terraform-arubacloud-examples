...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'caddy')."
  type        = string
  default     = "caddy"
...
variable "domain" {
  description = "Optional domain name for automatic HTTPS (e.g. 'example.com'). When set, Caddy obtains a Let's Encrypt certificate automatically. DNS A record must point to the VM's Elastic IP before apply."
  type        = string
  default     = ""
}
