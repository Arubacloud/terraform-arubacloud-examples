locals {
  ...
  tags               = ["superset", var.environment, "data-platform"]
...
resource "arubacloud_project" "this" {
  ...
  description = "Apache Superset data visualisation (${var.environment})"
  tags        = local.tags
}
...
# ── DBaaS (MySQL 8.0 — Superset metadata database) ────────────────────────────
...
resource "arubacloud_database" "superset" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  name       = "superset"
}

resource "arubacloud_dbaasuser" "superset" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  username   = "superset"
...
resource "arubacloud_databasegrant" "superset" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  database   = arubacloud_database.superset.id
  user_id    = arubacloud_dbaasuser.superset.id
  role       = "liteadmin"
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      ...
      db_name            = arubacloud_database.superset.name
      db_user            = arubacloud_dbaasuser.superset.username
  ...
  depends_on = [
    ...
    arubacloud_databasegrant.superset,
  ]
}
