locals {
  ...
  tags        = ["wazuh", var.environment, "security", "siem"]
}
...
resource "arubacloud_project" "this" {
  ...
  description = "Wazuh SIEM/XDR all-in-one (${var.environment})"
  tags        = local.tags
}
...
module "network" {
  ...
  vm_ingress_ports = {
    ...
    dashboard    = { port = "443", cidr = var.admin_cidr }
    agent_events = { port = "1514", cidr = var.agent_cidr }
    agent_enroll = { port = "1515", cidr = var.agent_cidr }
  }
}
