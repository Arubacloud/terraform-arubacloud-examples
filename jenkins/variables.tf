...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'jenkins')."
  type        = string
  default     = "jenkins"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO4A8 (4 vCPU / 8 GB) is the recommended minimum for Jenkins."
  type        = string
  default     = "CSO4A8"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Jenkins workspace and build artifacts are stored here."
  type        = number
  default     = 50

  validation {
    condition     = var.vm_disk_size_gb >= 30
    error_message = "vm_disk_size_gb must be at least 30 GB for Jenkins."
  }
}
...
variable "agent_cidr" {
  description = "CIDR allowed to reach the JNLP agent port 50000. Set to your agent IP range in production."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "domain" {
  description = "Custom domain for Jenkins (e.g. 'ci.example.com'). When set, Certbot provisions a Let's Encrypt TLS certificate. Leave empty to use the Elastic IP over HTTP."
  type        = string
  default     = ""
}
