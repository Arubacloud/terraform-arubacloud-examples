locals {
  ...
  tags        = ["mailcow", var.environment, "collaboration"]
}
...
resource "arubacloud_project" "this" {
  ...
  description = "Mailcow email server stack (${var.environment})"
  tags        = local.tags
}
...
module "network" {
  ...
  vm_ingress_ports = {
    ...
    smtp  = { port = "25", cidr = "0.0.0.0/0" }
    smtps = { port = "465", cidr = "0.0.0.0/0" }
    sub   = { port = "587", cidr = "0.0.0.0/0" }
    imaps = { port = "993", cidr = "0.0.0.0/0" }
    pop3s = { port = "995", cidr = "0.0.0.0/0" }
    sieve = { port = "4190", cidr = "0.0.0.0/0" }
  }
}
