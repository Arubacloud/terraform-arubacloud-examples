...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'mail')."
  type        = string
  default     = "mail"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO4A8 (4 vCPU / 8 GB) is the minimum for Mailcow."
  type        = string
  default     = "CSO4A8"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Mail storage grows over time — 80 GB is a reasonable start."
  type        = number
  default     = 80

  validation {
    condition     = var.vm_disk_size_gb >= 40
    error_message = "vm_disk_size_gb must be at least 40 GB for Mailcow."
  }
}
...
variable "mailcow_branch" {
  description = "Mailcow Git branch to install ('master' for stable)."
  type        = string
  default     = "master"
}
