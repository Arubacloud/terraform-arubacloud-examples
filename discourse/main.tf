locals {
  ...
  tags        = ["discourse", var.environment, "collaboration"]
  hostname    = var.hostname != "" ? var.hostname : module.network.vm_elastic_ip_address
}
...
resource "arubacloud_project" "this" {
  ...
  description = "Discourse community forum (${var.environment})"
  tags        = local.tags
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      ...
      smtp_pass_b64 = base64encode(var.smtp_password)
    })
  }
