...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'grafana')."
  type        = string
  default     = "grafana"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO4A8 (4 vCPU / 8 GB) is the recommended minimum for the full observability stack."
  type        = string
  default     = "CSO4A8"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Prometheus TSDB and Loki chunks are stored here — size according to your retention needs."
  type        = number
  default     = 80

  validation {
    condition     = var.vm_disk_size_gb >= 40
    error_message = "vm_disk_size_gb must be at least 40 GB for the observability stack."
  }
}
...
variable "domain" {
  description = "Custom domain for Grafana (e.g. 'metrics.example.com'). When set, Certbot provisions a Let's Encrypt TLS certificate. Leave empty to use the Elastic IP over HTTP."
  type        = string
  default     = ""
}
...
variable "prometheus_version" {
  description = "Prometheus release version (e.g. '3.0.1'). See https://github.com/prometheus/prometheus/releases."
  type        = string
  default     = "3.0.1"
}

variable "loki_version" {
  description = "Loki release version (e.g. '3.4.2'). Promtail uses the same version. See https://github.com/grafana/loki/releases."
  type        = string
  default     = "3.4.2"
}

variable "node_exporter_version" {
  description = "Node Exporter release version (e.g. '1.8.2'). See https://github.com/prometheus/node_exporter/releases."
  type        = string
  default     = "1.8.2"
}
