...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO4A8 (4 vCPU / 8 GB) is the recommended minimum for Airflow."
  type        = string
  default     = "CSO4A8"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. DAG files and logs are stored here."
  type        = number
  default     = 60
...
variable "db_password" {
  description = "MySQL password for the Airflow metadata database user. Min 8 characters, no newlines."
  type        = string
  sensitive   = true
...
variable "airflow_admin_user" {
  description = "Username for the initial Airflow admin account."
  type        = string
  default     = "admin"
}

variable "airflow_admin_password" {
  description = "Password for the initial Airflow admin account. Min 8 characters, no newlines."
  type        = string
  sensitive   = true

  validation {
    condition     = length(var.airflow_admin_password) >= 8 && !can(regex("\n", var.airflow_admin_password))
    error_message = "airflow_admin_password must be at least 8 characters and must not contain newlines."
  }
}

variable "airflow_admin_email" {
  description = "Email address for the initial Airflow admin account."
  type        = string
  default     = "admin@example.com"

  validation {
    condition     = can(regex("^[^@]+@[^@]+\\.[^@]+$", var.airflow_admin_email))
    error_message = "airflow_admin_email must be a valid email address."
  }
}

variable "airflow_version" {
  description = "Apache Airflow version to install (e.g. '2.10.4'). See https://pypi.org/project/apache-airflow/."
  type        = string
  default     = "2.10.4"
}

variable "domain" {
  description = "Custom domain for HTTPS (e.g. 'airflow.example.com'). Leave empty to use the Elastic IP over HTTP."
  type        = string
  default     = ""
}
