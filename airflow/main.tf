...
resource "arubacloud_project" "this" {
  ...
  description = "Apache Airflow workflow orchestration (${var.environment})"
  tags        = local.tags
}
...
# ── DBaaS (MySQL 8.0 — Airflow metadata database) ─────────────────────────────
...
resource "arubacloud_database" "airflow" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  name       = "airflow"
}

resource "arubacloud_dbaasuser" "airflow" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  username   = "airflow"
...
resource "arubacloud_databasegrant" "airflow" {
  project_id = arubacloud_project.this.id
  dbaas_id   = arubacloud_dbaas.this.id
  database   = arubacloud_database.airflow.id
  user_id    = arubacloud_dbaasuser.airflow.id
  role       = "liteadmin"
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  depends_on = [
    ...
    arubacloud_databasegrant.airflow,
  ]
}
