...
3. **Replace the self-signed certificate.** The quick-install generates a self-signed cert. For production, use a proper certificate — either via an ACME CA such as [Actalis ACME Certificates](https://guide.actalis.com/it/ssl/attivazione/acme) or Let's Encrypt (requires a domain) or your own CA.
