...
- **Actalis ACME (EAB) support** — all SSL/HTTPS examples (Traefik, Caddy, Nginx,
  GitLab, Ghost, Nextcloud, Mattermost, Keycloak, Discourse, Mailcow, Forgejo, Gitea)
  now expose optional `actalis_eab_kid` / `actalis_eab_hmac` variables for Italian
  CA certificates via Let's Encrypt EAB.
- **Defaults for all variables** — every module ships with sensible defaults so only
  `arubacloud_client_id` and `arubacloud_client_secret` are required to plan/apply.
- **HAProxy** (`haproxy/`) — demo nginx backend auto-deployed when no external backends
  are provided.
- **k3s HA** (`k3s-ha/`) — optional Managed MySQL 8.0 DBaaS as etcd external datastore.
- **Discourse** (`discourse/`) — optional `dev_smtp` mode with Mailpit for local SMTP
  testing.
- **Adminer** (`adminer/`) — now connects to a Managed MySQL DBaaS instead of a local
  database.
...
- **Jenkins** — APT keyring migrated to `/etc/apt/keyrings/` with the 2026 signing
  key; GPG dearmoring fixed; `runcmd` shell switched to bash for `pipefail`
...
[Unreleased]: https://github.com/Arubacloud/terraform-arubacloud-examples/compare/v0.5.0...HEAD
