...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'llm')."
  type        = string
  default     = "llm"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO2A4 (2 vCPU / 4 GB) is sufficient for the LiteLLM proxy."
  type        = string
  default     = "CSO2A4"
}
...
variable "api_cidr" {
  description = "CIDR allowed to reach the LiteLLM proxy API on port 4000. Restrict to your application servers."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "master_key" {
  ...
  type        = string
  sensitive   = true
  default     = "sk-K7m@P4z!L9xQ8rWs"

  validation {
    condition     = length(var.master_key) >= 8
    error_message = "master_key must be at least 8 characters."
  }
}

variable "openai_api_key" {
  description = "OpenAI API key to forward to OpenAI models. Leave empty if not using OpenAI."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "anthropic_api_key" {
  description = "Anthropic API key to forward to Claude models. Leave empty if not using Anthropic."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "ollama_base_url" {
  description = "URL of an Ollama server to include as a provider (e.g. 'http://10.0.0.10:11434'). Leave empty if not using Ollama."
  type        = string
  default     = ""
}
