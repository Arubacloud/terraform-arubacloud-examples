locals {
  ...
  tags        = ["elasticsearch", var.environment, "observability"]
}
...
resource "arubacloud_project" "this" {
  ...
  description = "Elasticsearch search and analytics (${var.environment})"
  tags        = local.tags
}
...
resource "arubacloud_cloudserver" "this" {
  ...
  settings = {
    ...
    user_data = templatefile("${path.module}/cloud-init.yaml.tpl", {
      ...
      elastic_pass_b64 = base64encode(var.elastic_password)
    })
  }
