...
variable "app_name" {
  description = "Short name used as part of all resource names (e.g. 'es')."
  type        = string
  default     = "es"
...
variable "vm_flavor" {
  description = "CloudServer flavor. CSO4A8 (4 vCPU / 8 GB) for development/light workloads; CSO8A16 (8 vCPU / 16 GB) recommended for production."
  type        = string
  default     = "CSO4A8"
}
...
variable "vm_disk_size_gb" {
  description = "Boot disk size in GB. Elasticsearch indexes can grow quickly — 100 GB is a reasonable starting point."
  type        = number
  default     = 100

  validation {
    condition     = var.vm_disk_size_gb >= 50
    error_message = "vm_disk_size_gb must be at least 50 GB for Elasticsearch."
  }
}
...
variable "admin_cidr" {
  description = "CIDR allowed to reach the Elasticsearch REST API on port 9200. Never expose Elasticsearch directly to the internet."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "elastic_password" {
  ...
  validation {
    ...
    error_message = "elastic_password must be at least 6 characters."
  }
}
